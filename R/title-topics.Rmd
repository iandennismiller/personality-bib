...
get_articles_written_by_author = function(g, name) {
  ...
  return(V(journal_1990)[author_relationships])
}
```
...
## Attributes of nodes in Journals graph

```{r}
list.vertex.attributes(journal_1990)
```

## Attributes of nodes in coauthors graph
...
for (author_idx in V(main_component_graph)[V(main_component_graph)$node_type == 'author']) {
  name = V(main_component_graph)[author_idx]$author_name
  articles = get_articles_written_by_author(journal_1990, name)
  for (article_idx in articles) {
    article = V(main_component_graph)[article_idx]
    print(article)
    # df_tmp = data.frame(
